...
\subsection{Experimental setup}
	In this double species experiment, fermionic $^6$Li and bosonic $^{133}$Cs are trapped and cooled inside a vacuum chamber. The atoms are emitted from an oven into a Zeeman slower. The slower consists of a magnetic field with a constant gradient along the propagation direction of the atoms. A laser beam pointing towards the atoms will therefore act as a velocity dependant force, as their energy levels are shifted until they are resonant with the beam.
	
	The atoms will move into the centre of the vacuum chamber, where they can be trapped and further slowed using a magneto-optical trap (MOT). Along the $x$, $y$ and $z$ axis, counter propagating laser beams are pointing at the centre of the trap, where the atoms will be confined. A magnetic field is implied, such that it is weak in the centre and strong on the outside. Therefore, fast or off-centre atoms become resonant with the laser light, pushing them inside.
	
	Inside the MOT, one can only reach temperatures around \SI{100}{\micro\kelvin}, so to further cool them down, the atoms are transferred into an optical dipole trap. The intensity of the dipole trap can be ramped, evaporatively cooling the atoms. Hereby, the faster atoms will leave the trap, so that the total temperature of the gas will be reduced up to a few \SI{}{\nano\kelvin}.
	...
	As the camera is also very sensitive to stray light, it has to be protected between measurements with a shutter, that covers the front end of the imaging path, such that no light will enter the camera, which is described in more detail in \refSec{shutter}
