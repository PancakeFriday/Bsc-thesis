...
\subsection{Experimental setup}
	In this double species experiment, fermionic $^6$Li and bosonic $^{133}$Cs are trapped and cooled inside a vacuum chamber. The atoms are emitted from an oven into a Zeeman slower. The slower consists of a decreasing magnetic field along the propagation direction of the atoms. A laser beam counter propagating the atoms will therefore act as a velocity dependant force, as their energy levels are shifted by the magnetic field until they are resonant with the beam.
	
	The atoms will move into the centre of the vacuum chamber, where they can be trapped and further slowed using a magneto-optical trap (MOT). Along the $x$, $y$ and $z$ axis, counter propagating laser beams are pointing at the centre of the trap, where the atoms will be confined. Magnetic coils in anti-Helmholtz configuration imply a field, such that it is weak in the centre and strong on the outside. Therefore, fast or off-centre atoms become resonant with the laser light, pushing them inside.
	
	The temperature inside the MOT is limited by the doppler temperature
	\begin{equation}
		T_D=\frac{\hbar\gamma}{2k_B} = \SI{141}{\micro\kelvin}
	\end{equation}
	where the lifetime of the D2 line is $\gamma=\SI{37}{\mega\hertz}$. In order to further cool them down, the atoms are transferred into an optical dipole trap. The intensity of the dipole trap can be ramped, evaporatively cooling the atoms. Hereby, only the slow atoms are kept in the trap, so that the total temperature of the gas will be reduced up to a few \SI{}{\nano\kelvin}.
	...
	As the camera is also very sensitive to stray light, it has to be protected from stray light and between measurements. This is achieved by covering the beam path in SM2 tubes and a shutter, which will then cover the whole area, so no light should enter the chip.
	
	With this setup, molecular Bose-Einstein condensates can be set up and imaged even at low atom numbers ($n=1000$).
...
The quantum efficiency is explained in \refCh{quantumeff} and is also highly important, as it describes how many photons are detected on a camera and is directly connected to the sensitivity. A higher quantum efficiency therefore means better results. \todo{QE of guppy?}
\todo{look up qe of andor}
...
Since we will be imaging two species, an absorption image for each species will be followed by two division images.
...
\refFig{slit_sketch} sketches the systematics behind a laser wave approaching a slit.
